        ...
        return declare(
            ...
            ],
            {
                ...
                _generateDetailArrayOfProteoformSequence: function(
                    ...
                    modificationLocation,variblePTMsArr
                ){
                    if(
                    ...
                    let detailArrayOfProteoformSequenceTempRever = [];
                    ...
                    diffFromRefSequenceResult.forEach(
                        function(item, index) {
                            if(typeof item === 'object' && item.value !== undefined)
                            {
                                ...
                                {
                                    // console.log("1 itemValue:",item.value);
                                    seqLenth = item.value.length;
                                    if (isReverseStrand === true) {
                                        // let newValue = "";
                                        // for(let i = item.value.length-1; i >= 0; i--) {
                                        //     newValue += item.value.charAt(i);
                                        // }
                                        // item.value = newValue;
                                        // console.log(seqLenth);
                                        for (let i = 0 ; i < mSScanMassMappingResultArray.length;++i) {
                                            if(mSScanMassMappingResultArray[i].type === 'B')
                                                {
                                                    mSScanMassMappingResultArray[i].position = seqLenth - mSScanMassMappingResultArray[i].position;
                                                }
                                                else if (mSScanMassMappingResultArray[i].type === 'Y')
                                                {
                                                    // console.log("seqlen:",seqLenth,"mSScanMassMappingResultArray[i].position:",mSScanMassMappingResultArray[i].position,
                                                    // "mSScanMassMappingResultArray[i].label:",mSScanMassMappingResultArray[i].label);
                                                    mSScanMassMappingResultArray[i].position = seqLenth + 1 - mSScanMassMappingResultArray[i].position;
                                                }
                                        }
                                    }  else {
                                        for (let i = 0 ; i < mSScanMassMappingResultArray.length;++i) {
                                
                                                if (mSScanMassMappingResultArray[i].type === 'Y')
                                                {
                                                    // console.log("seqlen:",seqLenth,"mSScanMassMappingResultArray[i].position:",mSScanMassMappingResultArray[i].position,
                                                    // "mSScanMassMappingResultArray[i].label:",mSScanMassMappingResultArray[i].label);
                                                    mSScanMassMappingResultArray[i].position += 1 ;
                                                }
                                        }
                                    }
                        
                                    // console.log("1 itemValue:",item.value,"lenth:",item.value.length);
                                    for(let i = 0; i < item.value.length; i++)
                                    {
                                        ...
                                        newNode.headOrTailFlag += ( index === diffFromRefSequenceResult.length - 1 && i === item.value.length - 1) ? 'TAIL' : '';
                                        ...
                                        realAminoAcidCount++;   // wz add 
                                        // console.log("newNode.aminoAcidCharacter:",newNode.aminoAcidCharacter,
                                        // "aminoAcidWithRemovedCharacterCount:",aminoAcidWithRemovedCharacterCount,
                                        // "realAminoAcidCount:",realAminoAcidCount);
                                        for(let i = 0; i < mSScanMassMappingResultArray.length; i++)
                                        {
                                            // if(mSScanMassMappingResultArray[i].position === aminoAcidWithRemovedCharacterCount) //wz add
                                            if (mSScanMassMappingResultArray[i].position === realAminoAcidCount) // wz add
                                            {
                                ...
                                else if(item.added === true)        // 添加的修饰  wz 修改
                                {
                                    // console.log("2 itemValue:",item.value);
                                    // wz modify -------deprect ------
                                    ...
                                    //     return false;
                                    // }
                                    // // wz add preNode 永远是同一个位置??
                                    // let prevNode = detailArrayOfProteoformSequence[ detailArrayOfProteoformSequence.length - 1]; // old
                                    // let refAddedCharacters = item.value;
                                    // if(item.modification !== undefined)
                                    // {
                                    //     // console.info("prevNode:",prevNode,"--",item.value,"--",item.modification);
                                    //     prevNode.modificationColor = item.modification.color;
                                    // }
                                    // prevNode.modification = refAddedCharacters;
                                    // ------- old modification --------
                                    // console.log("detailArrayOfProteoformSequence:",detailArrayOfProteoformSequence
                                    // ,"pendingInsertAtHead:",pendingInsertAtHead,"prevNode:",prevNode,
                                    // "refAddedCharacters:",refAddedCharacters,
                                    // "prevNode.modification:",prevNode.modification);

                                }
                                else if(item.removed === true)  //去除的序列
                                {
                                    // console.log("3 itemValue:",item.value);
                                    for(let i = 0; i < item.value.length; i++)
                                    {
                                        let newNode = dojoLang.clone(aminoAcidDetailTemplate);
                                        newNode.id = aminoAcidCharacterCount;
                    ...
                    // console.log(detailArrayOfProteoformSequence);
                    // wz add 将修饰固定在对应的氨基酸上
                    let realAcioCount = 0 ; 
                    let modArrLocation = 0 ; //指向modificationLocation 的位置
                    if (isReverseStrand === true) {
                        modArrLocation = modificationLocation.length -1;
                        for (let i = 0 ; i < detailArrayOfProteoformSequence.length;++i) {
                            if (detailArrayOfProteoformSequence[i].aminoAcidCharacter === '-') {
                                continue ; 
                            } else {
                                realAcioCount++ ; 
                                if (modificationLocation.length > 0 && modArrLocation >=0 ) {
                                    if (realAcioCount == seqLenth - modificationLocation[modArrLocation] + 1) {
                                        // console.log("i:",i,"realAcioCount:",realAcioCount," varible :",variblePTMsArr[modArrLocation],
                                        // "modArrLocation:",modArrLocation);
                                        detailArrayOfProteoformSequence[i].modification = variblePTMsArr[modArrLocation];
                                        modArrLocation--;
                                    }
                                }
                            }
                        ...
                        for (let i = 0 ; i < detailArrayOfProteoformSequence.length;++i) {
                            if (detailArrayOfProteoformSequence[i].aminoAcidCharacter === '-') {
                                ...
                                realAcioCount++ ; 
                                if (modificationLocation.length > 0 && modArrLocation < modificationLocation.length) {
                                    if (realAcioCount == modificationLocation[modArrLocation]) {
                                        // console.log("i:",i,"realAcioCount:",realAcioCount," varible :",variblePTMsArr[modArrLocation],
                                        // "modArrLocation:",modArrLocation);
                                        detailArrayOfProteoformSequence[i].modification = variblePTMsArr[modArrLocation];
                                        modArrLocation++;
                    ...
                    // {
                    //     s1 += detailArrayOfProteoformSequence[index].aminoAcidCharacter;
                    // }
                    // console.log("s1:",s1);

                    // // wz add 2022-12-15 将序列反过来
                    // if (isReverseStrand === true) {
                    //     for (let index = 0 ; index < detailArrayOfProteoformSequence.length ; ++index) {
                    //         if (detailArrayOfProteoformSequence[index].aminoAcidCharacter !== '-')
                    //         {
                    //             break ; 
                    //         } else {
                    //             let newNode = dojoLang.clone(detailArrayOfProteoformSequence[index]);
                    //             detailArrayOfProteoformSequenceTempRever.push(newNode);
                    //             // console.log("+,",newNode.aminoAcidCharacter);
                    //         }
                    //     }
                    //     for(let index = detailArrayOfProteoformSequence.length -1; index >= 0 ; --index)
                    //     {
                    //         if(detailArrayOfProteoformSequence[index].aminoAcidCharacter === '-') {
                    //             continue ; 
                    //         } else {
                    ...
                    //     }
                    //     let s3 = "";
                    //     for (let index = 0 ; index < detailArrayOfProteoformSequenceTempRever.length ; ++index)
                    //     {
                    //         s3 += detailArrayOfProteoformSequenceTempRever[index].aminoAcidCharacter;
                    //     }
                    //     console.log("s3:",s3);
                    //     for (let index = 0 ; index < detailArrayOfProteoformSequenceTempRever.length;++index) {
                    //         detailArrayOfProteoformSequence[index] = detailArrayOfProteoformSequenceTempRever[index];
                    //     }
                    //     // detailArrayOfProteoformSequence = detailArrayOfProteoformSequenceTempRever ; 
                    // }
                    // let s2 = "";
                    // for (let index = 0 ; index < detailArrayOfProteoformSequence.length ; ++index)
                    // {
                    //     s2 += detailArrayOfProteoformSequence[index].aminoAcidCharacter;
                    // }
                    // console.log("s2:",s2);
                ...
                _drawProteoformSequenceEventCallback: function(
                    ...
                    // wz add 定位修饰的位置
                    let modificationLocation = [];     //修饰的第一个氨基酸的位置
                    let variblePTMsArr = [] ; 
                    let modificationAciLoc = 0 ; 
                    let startLoc = false ; 
                    for (let i = 0 ; i < proteoformSequence.length ;++i) {
                        if (proteoformSequence.charAt(i) === '.' && startLoc === false) {
                            startLoc = true; 
                            continue ; 
                        }
                        if (startLoc === false) {
                        ...
                        if (proteoformSequence.charAt(i) === '(') {
                            modificationLocation.push(modificationAciLoc+1) ; 
                        }
                        if (proteoformSequence.charAt(i) === '[') {
                            let strPTM = proteoformSequence.charAt(i) ; 
                            while (proteoformSequence.charAt(i) != ']') {
                                i++;
                                strPTM += proteoformSequence.charAt(i);
                            }
                            variblePTMsArr.push(strPTM);
                            continue ; 
                        }
                        if (proteoformSequence.charAt(i) != '(' && proteoformSequence.charAt(i) != ')') {
                            modificationAciLoc++; 
                        }
                        
                    }
                    // console.log(modificationLocation);
                    // 
                    let detailArrayOfProteoformSequence = _this._generateDetailArrayOfProteoformSequence(
                        ...
                        scanId, diffFromRefSequenceResult, mSScanMassMappingResultArray,modificationLocation,variblePTMsArr
                    );
